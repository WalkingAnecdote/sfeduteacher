...
abstract class BaseRepository implements RepositoryContract
{
    ...
    /**
     * Get the first or fail specified model record from the database.
     *
     * @param array $columns
     *
     * @return Model|static
    ...
    public function customPagination($params = [], $columns = ['*'], $pageName = "page") {
        $page = $params['page'] ?? 1;
        if($page < 1) $page = 1;
        $perPage = $params['perPage'] ?? 10;

        $data_query = $this->newQuery()->eagerLoad()->setClauses()->setScopes();

        $models = $data_query->paginate($perPage, $columns, $pageName, $page);


        return [
            'paginatorInfo' => [
                'count' => count($models->items()),
